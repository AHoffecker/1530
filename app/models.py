...
db = SQLAlchemy()
# Set up each table with necessary columns
...
class WaitTime(db.Model):
    ...
    orderTime = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    receivedTime = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc)) 
    timestamp = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))    
